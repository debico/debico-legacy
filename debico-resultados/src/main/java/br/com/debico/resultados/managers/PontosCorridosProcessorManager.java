...
class PontosCorridosProcessorManager extends CampeonatoProcessorManagerSupport<CampeonatoPontosCorridos> {
	...
	public void execute(Context context) throws DebicoException {
		...
		if (!this.supports(context.getCampeonato())) {
			LOGGER.debug("[execute] Campeonato {} nao suportado.", context.getCampeonato());
	...
	private Context doStart(Context context) {
		LOGGER.debug("[doStart] Inicializando o processamento do campeonato {}", context.getCampeonato());
		this.decorateContext(context);
		try {
			this.processorPipeline.execute(context);
		} catch (DebicoException e) {
			// TODO: enviar email
			LOGGER.error("[doStart] Erro ao tentar iniciar o processamento do campeonato", e);
		}
	...
	private void decorateContext(final Context context) {
		if (context.getRodadas().isEmpty()) {
			context.setRodadas(rodadaService.selecionarRodadasNaoCalculadas(context.getCampeonato()));
		}
	}
