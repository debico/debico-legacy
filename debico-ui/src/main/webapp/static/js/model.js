...
function SerializeJSON() {
	...
	this.placar = function(model) {
		return ko.toJSON(model, function(key, value) {
			// ref:
			// http://stackoverflow.com/questions/12461037/knockout-serialization-with-ko-tojson-how-to-ignore-properties-that-are-null
...
function Placar(json, idPartida) {
	...
	self.isValido = function() {
		if (typeof self.golsMandante == "function") {
			if (typeof self.golsVisitante() !== "undefined"
					&& typeof self.golsMandante() !== "undefined") {
...
function Palpite(json) {
	...
	self.placar = (json) ? (new Placar(json.placar, json.idPartida))
			: (new Placar());
...
function Partida(json) {
	...
	self.dataHoraPartida = ko.observable(moment(json.dataHoraPartida).format(
			"DD/MM/YYYY HH:mm"));
	self.dataHoraPartidaDisplay = moment(json.dataHoraPartida).format(
	...
	self.hasPlacarProcessado = ko.pureComputed(function() {
		return this.hasPlacar && this.computadaCampeonato;
	}, this);
}
...
function PartidaPalpite(json) {
	...
	this.palpite = (json.palpite !== null) ? (new Palpite(json.palpite))
			: (new Palpite());
}
