...
class CampeonatoPontosCorridosServiceImpl extends CampeonatoServiceImpl implements
		CampeonatoPontosCorridosService {
	...
	public Rodada selecionarRodadaAtual(CampeonatoPontosCorridos campeonatoPontosCorridos) {
	    LOGGER.debug("[selecionarRodadaAtual] Tentando recuperar a rodada atual do campeonato {}", campeonatoPontosCorridos);
	    checkNotNull(campeonatoPontosCorridos);

		final Rodada rodada = rodadaDAO.selecionarRodadaAtual(campeonatoPontosCorridos);
	...
	public Rodada selecionarUltimaRodada(CampeonatoPontosCorridos campeonatoPontosCorridos) {
	    LOGGER.debug("[selecionarUltimaRodada] Tentando recuperar a ultima rodada do campeonato {}", campeonatoPontosCorridos);
        checkNotNull(campeonatoPontosCorridos);
	...
	public List<PartidaRodada> selecionarPartidasRodada(int idRodada) {
	    LOGGER.debug("[selecionarPartidasRodada] Tentando recuperar as partidas da rodada {}", idRodada);
	...
	@Cacheable(CacheKeys.PARTIDAS_RODADA)
	public List<PartidaRodada> selecionarPartidasRodada(int ordinalRodada, final String campeonatoPermalink) {
	    LOGGER.debug("[selecionarPartidasRodada] Tentando recuperar as partidas da rodada com ordinal {} e campeonato {}", ordinalRodada, campeonatoPermalink);
	    checkArgument(ordinalRodada > 0, "O ordinal deve ser maior que 0.");
	    checkNotNull(emptyToNull(campeonatoPermalink));
	    
	    return partidaDAO.selecionarPartidasPorRodada(ordinalRodada, campeonatoPermalink);
	}

	@Cacheable(CacheKeys.TABELA_CAMPEONATO)
	public List<PontuacaoTime> selecionarTabela(CampeonatoPontosCorridos campeonatoPontosCorridos) {
	    LOGGER.debug("[selecionarTabela] Tentando selecionar a Tabela do campeonato {}", campeonatoPontosCorridos);
	    
	    checkNotNull(campeonatoPontosCorridos);
		List<PontuacaoTime> ranking = pontuacaoTimeDAO.selecionarPontuacaoCampeonato(campeonatoPontosCorridos);
	...
	public void definirFaseUnica(CampeonatoPontosCorridos campeonatoPontosCorridos) {
		checkNotNull(campeonatoPontosCorridos);
		this.validarEstruturaCampeonato(campeonatoPontosCorridos);

		campeonatoPontosCorridos = (CampeonatoPontosCorridos) this.selecionarCampeonato(campeonatoPontosCorridos.getId());
		...
		for (Time timeA : times) {
			...
			for (Time timeB : times) {
				if (!timeA.equals(timeB)) {
					PartidaRodada partidaRodada = new PartidaRodada(timeA, timeB);
	 ...
	 */
	protected void validarEstruturaCampeonato(final CampeonatoPontosCorridos campeonatoPontosCorridos) {
		checkArgument(campeonatoPontosCorridos.getId() > 0, "Eh preciso um campeonato carregado e valido.");
