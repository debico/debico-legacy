 ...
 * <p/>
 * Deixa os <code>hooks</code> prontos para utilizarem do motor de regras para
 * processar o resultado das partidas.
 * <p/>
 * Todas as partidas já são previamente definidas com o status correto de acordo
 * com o placar, assim como os pontos adquiridos pelos apostadores no bolão.
 * <p/>
 * É sugerido que todas as classes de processamento utilizem dessa classe para
 * realizar as implementações.
 * 
 * @author r_fernandes
 * @since 0.1
 */
abstract class AbstractResultadosService<C extends Campeonato> implements
        ResultadosService<C> {

    private final Logger LOGGER = LoggerFactory
            .getLogger(AbstractResultadosService.class);
    ...
    /**
     * Realiza o processamento da {@link Rodada} especificada. São as partidas
     * que já foram determinadas o placar e que ainda não tem status. Todos os
     * palpites também são computados.
     */
    public final List<PartidaRodada> processar(final C campeonato,
            final Rodada rodada) {
        // sanity check
        checkNotNull(rodada);
        // define o status das partidas na rodada.
        List<PartidaRodada> partidas = this.calculoPartidasService
        ...
        LOGGER.debug(
                "[processar] Nao ha partidas para serem processadas pela rodada {} no campeonato {}",
                rodada, campeonato);

        return Collections.emptyList();
    }

    public final List<PartidaChave> processar(final C campeonato,
            final Chave chave) {
        // sanity check
        checkNotNull(chave);
        // define o status das partidas na rodada.
        List<PartidaChave> partidas = this.calculoPartidasService
                .definirStatusPartidas(chave);
    ...
    public final List<PartidaRodada> processar(C campeonato) {
        ...
        List<Rodada> rodadas = rodadaService
     ...
     * 
     * @param partidas
     *            já processadas e com placar.
     */
    protected abstract void doEfetuarCalculoPontosTimeRodada(
            final Rodada rodada, final List<PartidaRodada> partidas);
     ...
     *            já processadas e com placar.
     */
    protected abstract void doEfetuarCalculoResultadoTimeChave(
            final Chave chave, final List<PartidaChave> partidas);
