...
class ProcessadorRegrasBolaoImpl implements ProcessadorRegrasBolaoFeedback {
    ...
    private static final Logger LOGGER = LoggerFactory
	    .getLogger(ProcessadorRegrasBolaoImpl.class);
    ...
    public List<ProcessadorRegrasBolaoLog> recuperarLogsProcessamento(
	    Date dataProcessamento) {
	LocalDate dataMeiaNoite = new DateTime(dataProcessamento).toLocalDate();
	LOGGER.debug(
		"[recuperarLogsProcessamento] Recuperando os logs a partida da data {}.",
		dataMeiaNoite);
    ...
    protected void processarResultadosPontosCorridos(
	    final CampeonatoPontosCorridos campeonatoPontosCorridos) {
	final List<Rodada> rodadas = rodadaService
		.selecionarRodadasNaoCalculadas(campeonatoPontosCorridos);
	LOGGER.debug(
	...
	for (Rodada rodada : rodadas) {
	    try {
		List<PartidaRodada> partidas = campeonatoPCResultados
			.processar(campeonatoPontosCorridos, rodada);

		if (!partidas.isEmpty()) {
		    calculoPalpitesService.computarPalpites(
			    campeonatoPontosCorridos, partidas);
		    logDAO.create(new ProcessadorRegrasBolaoLog(
			    campeonatoPontosCorridos, rodada));
		    LOGGER.debug(
			    "[processarResultadosPontosCorridos] Rodada {} do campeonato {} processada com sucesso.",
			    rodada, campeonatoPontosCorridos);
		}
	    } catch (Exception ex) {
		LOGGER.error("Ops! Ocorreu um erro durante o processamento", ex);
		logDAO.create(new ProcessadorRegrasBolaoLog(
			campeonatoPontosCorridos, rodada, ex));
	    }
