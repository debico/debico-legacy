...
class DesempenhoApostadorServiceImpl implements DesempenhoApostadorService {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(DesempenhoApostadorServiceImpl.class);
    ...
    public Set<PontuacaoRodadaApostadorSerie> recuperarDesempenhoTop10PontuacaoPorRodada(
            Campeonato campeonato, int idUsuario) {
        ...
        final List<PontuacaoRodadaApostador> data = pontuacaoRodadaApostadorDAO
                .recuperarTop10Apostadores(campeonato);
        boolean inTop10 = false;

        LOGGER.debug(
                "[recuperarDesempenhoPontuacaoPorRodada] Tentando recuperar o desempenho do apostador {}",
                apostador.getId());

        for (PontuacaoRodadaApostador r : data) {
            PontuacaoRodadaApostadorSerie serie = new PontuacaoRodadaApostadorSerie(
                    r.getNomeApostador(), r.getIdApostador());
        ...
        LOGGER.debug(
                "[recuperarDesempenhoPontuacaoPorRodada] Recuperado o Top 10 {}",
                pontuacao);
        ...
        if (!inTop10) {
            LOGGER.debug(
                    "[recuperarDesempenhoPontuacaoPorRodada] Opa! O apostador '{}' nao estao no TOP 10, recuperando o seu indice",
                    apostador.getId());

            pontuacao.add(this.recuperarDesempenhoPontuacaoPorRodada(
                    campeonato, apostador));
        }
    ...
    public PontuacaoRodadaApostadorSerie recuperarDesempenhoPontuacaoPorRodada(
            Campeonato campeonato, int idUsuario) {
        checkArgument(idUsuario > 0, "Opa! Apostador NULO!");
        final Apostador apostador = apostadorService
                .selecionarApostadorPorIdUsuario(idUsuario);
        return this
                .recuperarDesempenhoPontuacaoPorRodada(campeonato, apostador);
    }

    protected PontuacaoRodadaApostadorSerie recuperarDesempenhoPontuacaoPorRodada(
            Campeonato campeonato, Apostador apostador) {
        final List<PontuacaoRodadaApostador> desempenho = pontuacaoRodadaApostadorDAO
                .recuperarPontuacaoApostador(campeonato, apostador.getId());

        if (desempenho.size() > 0) {
            return new PontuacaoRodadaApostadorSerie(desempenho);
        } else {
            return new PontuacaoRodadaApostadorSerie(apostador.getNome(),
                    apostador.getId());
        }
    }
