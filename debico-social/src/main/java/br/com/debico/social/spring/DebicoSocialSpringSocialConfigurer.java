...
/**
 * Re-implementação por razão deste BUG:
 * https://jira.spring.io/browse/SOCIAL-456
 * 
 * TODO: ficar de olho na resolução.
 * 
 * @author Ricardo Zanini (ricardozanini@gmail.com)
 *
 */
public class DebicoSocialSpringSocialConfigurer extends SpringSocialConfigurer {
    ...
    public DebicoSocialSpringSocialConfigurer() {
	super();
    }

    /**
     * Sets the {@link UserIdSource} to use for authentication. Defaults to
     * {@link AuthenticationNameUserIdSource}.
     * 
     * @param userIdSource
     *            the UserIdSource to use when authenticating
	...
	super.userIdSource(userIdSource);
	return this;
    }
	...
	super.postLoginUrl(postLoginUrl);
	return this;
    }

    /**
     * If true, always redirect to postLoginUrl, even if a pre-signin target is
     * in the request cache.
     * 
     * @param alwaysUsePostLoginUrl
     *            if true, always redirect to the postLoginUrl
     * @return this SpringSocialConfigurer for chained configuration
     */
    public SpringSocialConfigurer alwaysUsePostLoginUrl(
	    boolean alwaysUsePostLoginUrl) {
	this.alwaysUsePostLoginUrl = alwaysUsePostLoginUrl;
	super.alwaysUsePostLoginUrl(alwaysUsePostLoginUrl);
	return this;
    }
     ...
     * 
     * @param postFailureUrl
     *            the URL to redirect to after a failed login
     * @return this SpringSocialConfigurer for chained configuration
     */
    public SpringSocialConfigurer postFailureUrl(String postFailureUrl) {
	this.postFailureUrl = postFailureUrl;
	super.postFailureUrl(postFailureUrl);
	return this;
    }

    /**
     * Sets the URL to land on after an authentication failure so that the user
     * can register with the application.
     * 
     * @param signupUrl
     *            the URL to redirect to after an authentication failure
     * @return this SpringSocialConfigurer for chained configuration
     */
     ...
     * 
     * @param connectionAddedRedirectUrl
     *            the URL to redirect after a connection was added
     * @return this SpringSocialConfigurer for chained configuration
     */
    public SpringSocialConfigurer connectionAddedRedirectUrl(
	    String connectionAddedRedirectUrl) {
	this.connectionAddedRedirectUrl = connectionAddedRedirectUrl;
	super.connectionAddedRedirectUrl(connectionAddedRedirectUrl);
	return this;
    }

    /**
     * Sets the URL to redirect to if authentication fails or if authorization
     * is denied by the user.
     * 
     * @param defaultFailureUrl
     *            the URL to redirect to after an authentication fail or
     *            authorization deny
     * @return this SpringSocialConfigurer for chained configuration
     */
    public SpringSocialConfigurer defaultFailureUrl(String defaultFailureUrl) {
	this.defaultFailureUrl = defaultFailureUrl;
	super.defaultFailureUrl(defaultFailureUrl);
	return this;
    }

    @Override
    public void configure(HttpSecurity http) throws Exception {
	ApplicationContext applicationContext = http
		.getSharedObject(ApplicationContext.class);
	UsersConnectionRepository usersConnectionRepository = getDependency(
		applicationContext, UsersConnectionRepository.class);
	SocialAuthenticationServiceLocator authServiceLocator = getDependency(
		applicationContext, SocialAuthenticationServiceLocator.class);
	SocialUserDetailsService socialUsersDetailsService = getDependency(
		applicationContext, SocialUserDetailsService.class);

	SocialAuthenticationFilter filter = new SocialAuthenticationFilter(
		http.getSharedObject(AuthenticationManager.class),
		userIdSource != null ? userIdSource
			: new AuthenticationNameUserIdSource(),
		usersConnectionRepository, authServiceLocator);

	RememberMeServices rememberMe = http
	...
	http.authenticationProvider(
		new SocialAuthenticationProvider(usersConnectionRepository,
			socialUsersDetailsService)).addFilterBefore(
		postProcess(filter),
		AbstractPreAuthenticatedProcessingFilter.class);

    }

    private <T> T getDependency(ApplicationContext applicationContext,
	...
	try {
	    ...
	    throw new IllegalStateException(
		    "SpringSocialConfigurer depends on "
