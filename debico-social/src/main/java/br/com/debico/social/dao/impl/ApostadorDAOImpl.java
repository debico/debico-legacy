...
class ApostadorDAOImpl extends AbstractJPADAO<Apostador, Integer> implements ApostadorDAO {
    ...
    public List<Apostador> selecionarApostadoresSemPalpite(final Campeonato campeonato, Date dataInicioPartida, Date dataFimPartida) {
    	final CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
    	// principal
    	final CriteriaQuery<Apostador> query = cb.createQuery(Apostador.class);
    	...
    	subquery2.where(
    			cb.and(cb.equal(pontuacao.get("apostador"), palpite.get("apostador")),
