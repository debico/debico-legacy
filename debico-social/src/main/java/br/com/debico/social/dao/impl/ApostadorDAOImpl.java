...
class ApostadorDAOImpl extends AbstractJPADao<Apostador, Integer> implements
        ApostadorDAO {
    ...
    public List<Apostador> procurarPorParteNome(String parteNome) {
        ...
        query.select(
                cb.construct(Apostador.class, apostador.get(Apostador_.id),
                        apostador.get(Apostador_.nome))).where(
                cb.like(apostador.get(Apostador_.nome),
                        String.format("%%%s%%", parteNome)));

        return getEntityManager().createQuery(query).setMaxResults(20)
                .getResultList();
    }
    ...
    public Apostador selecionarPorIdUsuario(int idUsuario) {
        ...
        final Join<Apostador, Usuario> usuario = (Join<Apostador, Usuario>) apostador
                .fetch(Apostador_.usuario, JoinType.INNER);

        // fetch
        apostador.join(Apostador_.opcoes).fetch(ApostadorOpcoes_.timeCoracao,
                JoinType.LEFT);

        query.select(apostador).where(cb.equal(usuario.get("id"), idUsuario));

        return DataAccessUtils.singleResult(getEntityManager().createQuery(
                query).getResultList());
    }
    ...
    public Apostador selecionarPorSocialId(String socialUserId) {
        ...
        query.select(apostador).where(
                cb.equal(usuario.get("socialUserId"), socialUserId));
    ...
    public Apostador selecionarPorEmail(String email) {
        return DataAccessUtils
                .singleResult(getEntityManager()
                        .createQuery(
                                "SELECT a FROM Apostador a WHERE usuario.email = :email",
                                Apostador.class).setParameter("email", email)
                        .getResultList());
    }
    ...
    public List<Apostador> selecionarApostadoresSemPalpite(
            final Campeonato campeonato, Date dataInicioPartida,
            Date dataFimPartida) {
        final CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        // principal
        final CriteriaQuery<Apostador> query = cb.createQuery(Apostador.class);
        ...
        final Root<ApostadorPontuacao> pontuacao = query
                .from(ApostadorPontuacao.class);
        final Join<ApostadorPontuacao, Apostador> apostador = pontuacao
                .join("apostador");
        ...
        subquery2.where(cb.and(cb.equal(pontuacao.get("apostador"),
                palpite.get("apostador")), cb.between(
                joinSub.<Date> get("dataHoraPartida"), dataInicioPartida,
                dataFimPartida)));

        final List<Predicate> predicates = new ArrayList<Predicate>();

        predicates.add(cb.equal(
                apostador.get("opcoes").get("lembretePalpites"), true));
        predicates.add(cb.isNotNull(apostador.get("usuario").get("email")));
        predicates.add(cb.equal(fase.get("campeonato"),
                pontuacao.get("campeonato")));
        predicates.add(cb.between(partida.<Date> get("dataHoraPartida"),
                dataInicioPartida, dataFimPartida));
        predicates.add(cb.equal(pontuacao.get("campeonato"), campeonato));
        predicates.add(cb.not(cb.exists(subquery2)));

        query.distinct(true).select(
                cb.construct(Apostador.class, apostador.get("id"),
                        apostador.get("nome"),
